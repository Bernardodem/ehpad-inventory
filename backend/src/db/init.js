...
const DB_PATH = path.join(__dirname, '../../data/ehpad.db');
...
export function initDb() {
  // sync mkdir
  import('fs').then(({ mkdirSync }) => mkdirSync(path.dirname(DB_PATH), { recursive: true }));
  ...
  if (!existingAdmin) {
    ...
    const hashIDEC = bcrypt.hashSync('Idec2024!', 10);
    db.prepare(`INSERT INTO users (id, username, password_hash, full_name, role) VALUES (?, ?, ?, ?, ?)`).run(
  ...
  if (countProduits.c === 0) {
    ...
    const produits = [
      ...
      ['Brumisateur d\'eau minérale', '400mL', 'Divers', '47797', 'CS Médical', 'Boite de 9', null, 3, '1'],
      ...
      ['Protection de thermomètre auriculaire', null, 'Examen', '588', 'CS Médical', 'carton de 800', null, 2, '1'],
      ...
      ['Gants de toilette à UU', null, 'Hygiène', '76387', 'CS Médical', 'Sachet de 50', 6, 6, '2'],
      ['Huile protectrice', '200mL', 'Hygiène', '56020', 'CS Médical', 'unité', 10, 10, '5'],
      ...
      ['Sac à linge sale (50L)', null, 'Protection', '44100', 'CS Médical', 'Rouleau de 50', 100, 200, '50'],
